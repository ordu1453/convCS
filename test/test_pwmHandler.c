...
void test_Precharge_Init(void)
{
	...
	TEST_ASSERT_EQUAL_INT(STATE_DISABLE, currentPWMState);
	TEST_ASSERT_EQUAL_INT(STATE_INIT, currentState);
...
void test_Precharge_DischargeWithNoError(void)
{
	...
	TEST_ASSERT_EQUAL_INT(STATE_ENABLE, currentPWMState);
	TEST_ASSERT_EQUAL_INT(STATE_DISCHARGE, currentState);
}
